...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      // 启动图谱数据轮询（独立于任务状态轮询）
      startGraphPolling(20000)  // 每20秒轮询一次，避免429错误
...
const stopGraphPolling = () => {
  if (graphPollTimer) {
    ...
    graphPollTimer = null
    console.log('Graph polling stopped')
  }
}

// 启动图谱数据轮询（带延迟）
const startGraphPolling = (interval = 10000) => {
  // 立即停止现有的轮询
  stopGraphPolling()
  ...
  console.log(`Graph polling started with interval ${interval}ms`)
}
  ...
  try {
    ...
    if (projectResponse.success && projectResponse.data.graph_id) {
      ...
      if (graphResponse.success && graphResponse.data) {
        // 检查是否是速率限制错误
        if (graphResponse.data.error === 'rate_limit') {
          console.warn('Zep 429 速率限制，暂停图谱轮询')
          stopGraphPolling()
          // 30秒后重试
          setTimeout(() => {
            console.log('30秒后重试图谱轮询')
            startGraphPolling(20000)
          }, 30000)
          return
        }
        ...
        const shouldUpdate = (
          ...
          !graphData.value ||
          JSON.stringify(newData) !== JSON.stringify(graphData.value)
        )
        ...
        // API 返回成功但 data 为空
        console.warn('Graph data is empty or invalid:', graphResponse)
        if (graphData.value) {
    ...
    console.error('Graph data fetch failed:', err.message || 'not ready')
    // 发生错误时清空图谱数据
    if (err.response?.status === 429) {
      console.warn('Rate limit exceeded, stopping graph polling')
      stopGraphPolling()
    }
    graphData.value = null
  }
...
const pollTaskStatus = async (taskId) => {
  try {
    ...
    if (response.success) {
      ...
      // 实时更新：获取增量图谱数据并更新左侧可视化
      if (task.progress_detail?.incremental_graph) {
        const incGraph = task.progress_detail.incremental_graph
        console.log('📊 收到增量图谱数据:', incGraph.node_count, '节点,', incGraph.edge_count, '边')
        
        // 更新图谱数据（实时显示构建过程）
        if (incGraph.nodes && incGraph.nodes.length > 0) {
          // 先确保 currentPhase 是 1（构建阶段）
          if (currentPhase.value !== 1) {
            currentPhase.value = 1
          }
          
          // 更新图谱数据（watch 会自动触发 renderGraph）
          graphData.value = {
            nodes: incGraph.nodes,
            edges: incGraph.edges || [],
            node_count: incGraph.node_count,
            edge_count: incGraph.edge_count
          }
          
          console.log('✅ graphData 已更新，节点数:', incGraph.nodes.length)
        }
      }
...
const renderGraph = () => {
  ...
  if (!graphSvg.value) {
    console.error('❌ Cannot render: graphSvg.value is null!')
    return
  }
  if (!graphData.value) {
    console.error('❌ Cannot render: graphData.value is null!')
    return
  }
...
watch(graphData, (newVal, oldVal) => {
  console.log('🔍 watch(graphData) 触发:', { 
    hasValue: !!newVal, 
    nodesCount: newVal?.nodes?.length,
    oldVal: !!oldVal,
    currentPhase: currentPhase.value,
    graphSvgExists: !!graphSvg.value
  })
  if (newVal) {
    nextTick(() => {
      console.log('🎨 调用 renderGraph(), graphSvg:', !!graphSvg.value)
      renderGraph()
    })
  }
}, { immediate: false })
