<template>
  <div class="process-page">
    <!-- 顶部导航�?-->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">TRACEBACK 溯·源</div>
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            ...
            <div class="action-buttons">
                ...
                <button class="action-btn" @click="toggleFullScreen" :title="isFullScreen ? '退出全�? : '全屏显示'">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">生成完成后将自动开始构建图�?/p>
          </div>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  上传文档后，LLM分析文档内容，自动生成适合舆论模拟的本体结构（实体类型 + 关系类型�?                </div>
              </div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">生成的实体类�?({{ projectData.ontology.entity_types?.length || 0 }})</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">生成的关系类�?({{ projectData.ontology.relation_types?.length || 0 }})</div>
        ...
        <div class="project-panel">
          ...
          <div class="project-details" v-if="projectData">
            ...
            <div class="project-item">
              <span class="item-label">模拟需�?/span>
...
// 当前项目ID（可能从'new'变为实际ID�?const currentProjectId = ref(route.params.projectId)
...
const currentPhase = ref(-1) // -1: 上传�? 0: 本体生成�? 1: 图谱构建, 2: 完成
const selectedItem = ref(null) // 选中的节点或�?const isFullScreen = ref(false)

// DOM引用
const graphContainer = ref(null)
const graphSvg = ref(null)

// 轮询定时�?let pollTimer = null

// 计算属�?const statusClass = computed(() => {
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 1) return '图谱构建�?
  if (currentPhase.value === 0) return '本体生成�?
  return '初始化中'
})
...
const getPhaseStatusText = (phase) => {
  if (currentPhase.value > phase) return '已完�?
...
// 初始�?- 处理新建项目或加载已有项�?const initProject = async () => {
...
const handleNewProject = async () => {
  ...
  try {
    ...
    ontologyProgress.value = { message: '正在上传文件并分析文�?..' }
    ...
    if (response.success) {
      // 清除待上传数�?      clearPendingUpload()
      
      // 更新项目ID和数�?      currentProjectId.value = response.data.project_id
      projectData.value = response.data
      
      // 更新URL（不刷新页面�?      router.replace({
        name: 'Process',
      ...
      // 自动开始图谱构�?      await startBuildGraph()
    } else {
      error.value = response.error || '本体生成失败'
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = '项目初始化失�? ' + (err.message || '未知错误')
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // 自动开始图谱构�?      if (response.data.status === 'ontology_generated' && !response.data.graph_id) {
...
// 开始构建图�?const startBuildGraph = async () => {
  try {
    currentPhase.value = 1
    // 设置初始进度
    buildProgress.value = {
    ...
    if (response.success) {
      buildProgress.value.message = '图谱构建任务已启�?..'
      ...
      // 启动任务状态轮�?      startPollingTask(taskId)
    } else {
...
// 图谱数据轮询定时�?let graphPollTimer = null

// 启动图谱数据轮询
const startGraphPolling = () => {
  // 立即获取一�?  fetchGraphData()
  
  // �?10 秒自动获取一次图谱数�?  graphPollTimer = setInterval(async () => {
...
// 轮询任务状�?const startPollingTask = (taskId) => {
  // 立即执行一次查�?  pollTaskStatus(taskId)
...
// 查询任务状�?const pollTaskStatus = async (taskId) => {
  try {
    ...
    if (response.success) {
      ...
      buildProgress.value = {
        ...
        message: task.message || '处理�?..'
      }
      
      console.log('Task status:', task.status, 'Progress:', task.progress)
      
      if (task.status === 'completed') {
        console.log('�?图谱构建完成，正在加载完整数�?..')
        
        stopPolling()
        stopGraphPolling()
        currentPhase.value = 2
        
        // 更新进度显示为完成状�?        buildProgress.value = {
          progress: 100,
          message: '构建完成，正在加载图�?..'
        }
        ...
        if (projectResponse.success) {
          ...
          // 最终加载完整图谱数�?          if (projectResponse.data.graph_id) {
...
const renderGraph = () => {
  ...
  // 创建节点ID集合用于过滤有效�?  const nodeIds = new Set(nodes.map(n => n.id))
  ...
  // 绘制边（包含可点击的透明宽线�?  const linkGroup = g.append('g')
    .attr('class', 'links')
    .selectAll('g')
    .data(edges)
    .enter()
  ...
  // 可见的细�?  const link = linkGroup.append('line')
    .attr('stroke', '#ccc')
    .attr('stroke-width', 1.5)
    .attr('stroke-opacity', 0.6)
  
  // 透明的宽线用于点�?  linkGroup.append('line')
    .attr('stroke', 'transparent')
    .attr('stroke-width', 10)
  
  // 边标�?  const linkLabel = g.append('g')
  ...
  // 点击空白处关闭详情面�?  svg.on('click', () => {
    closeDetailPanel()
  })
  
  simulation.on('tick', () => {
    // 更新所有边的位置（包括可见线和透明点击区域�?    linkGroup.selectAll('line')
