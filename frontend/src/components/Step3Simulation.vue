...
const emit = defineEmits(['go-back', 'next-step', 'add-log', 'update-status', 'pause', 'return-home', 'update-simulating-status'])
...
const doStartSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 通知父组件模拟开始
      emit('update-simulating-status', true)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        ...
        stopPolling()
        // 通知父组件模拟结束
        emit('update-simulating-status', false)
        emit('update-status', 'completed')
      }
    }
  } catch (err) {
