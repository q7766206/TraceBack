...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // 从Step2传入的最大轮�?  minutesPerRound: {
    type: Number,
    default: 30 // 默认每轮30分钟
  },
  projectData: Object,
...
const phase = ref(0) // 0: 未开�? 1: 运行�? 2: 已完�?const isStarting = ref(false)
const isStopping = ref(false)
const startError = ref(null)
const runStatus = ref({})
const allActions = ref([]) // 所有动作（增量累积�?const actionIds = ref(new Set()) // 用于去重的动作ID集合
const scrollContainer = ref(null)

// Computed
// 按时间顺序显示动作（最新的在最后面，即底部�?const chronologicalActions = computed(() => {
  return allActions.value
})

// 各平台动作计�?const twitterActionsCount = computed(() => {
...
// 格式化模拟流逝时间（根据轮次和每轮分钟数计算�?const formatElapsedTime = (currentRound) => {
...
// Twitter平台的模拟流逝时�?const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Reddit平台的模拟流逝时�?const redditElapsedTime = computed(() => {
...
// 重置所有状态（用于重新启动模拟�?const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
...
const doStartSimulation = async () => {
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true,  // 强制重新开�?      enable_graph_memory_update: true  // 开启动态图谱更�?    }
    
    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`设置最大模拟轮�? ${props.maxRounds}`)
    }
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 额外检查：如果后端还没来得及更�?runner_status，但平台已经报告完成
      // 通过检�?twitter_completed �?reddit_completed 状态判�?      const platformsCompleted = checkPlatformsCompleted(data)
    ...
    console.warn('获取运行状态失�?', err)
  }
}

// 检查所有启用的平台是否已完�?const checkPlatformsCompleted = (data) => {
  // 如果没有任何平台数据，返�?false
  if (!data) return false
  
  // 检查各平台的完成状�?  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // 如果至少有一个平台完成了，检查是否所有启用的平台都完成了
  // 通过 actions_count 判断平台是否被启用（如果 count > 0 �?running 曾为 true�?  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // 使用 all_actions 获取完整的动作列�?      const serverActions = res.data.all_actions || []
      
      // 增量添加新动作（去重�?      let newActionsAdded = 0
...
const handleNextStep = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`�?报告生成任务已启�? ${reportId}`)
      
      // 跳转到报告页�?      router.push({ name: 'Report', params: { reportId } })
    } else {
