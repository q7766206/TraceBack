<template>
  <div class="graph-panel">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <!-- 加载状态 — 光点汇聚动画 -->
      <div v-else-if="loading" class="graph-state graph-state--fullscreen">
        <canvas ref="particleCanvas" class="particle-canvas"></canvas>
        <div class="converge-overlay">
          <div class="converge-text">Graph Loading</div>
        </div>
      </div>
      
      <!-- 等待/空状态 — 光点汇聚动画 -->
      <div v-else class="graph-state graph-state--fullscreen">
        <canvas ref="particleCanvas" class="particle-canvas"></canvas>
        <div class="converge-overlay">
          <div class="converge-text">Awaiting Ontology</div>
          <div class="converge-sub">Preparing knowledge graph structure...</div>
        </div>
      </div>
    </div>
...
onMounted(() => {
  ...
  nextTick(() => initConvergeAnimation())
})
...
function initConvergeAnimation() {
  ...
  // 粒子颜色池 — 柔和的科技色
  const colors = [
    'rgba(233, 30, 99, ALPHA)',   // 粉红（主题色）
    'rgba(156, 39, 176, ALPHA)',  // 紫色
    'rgba(63, 81, 181, ALPHA)',   // 靛蓝
    'rgba(0, 188, 212, ALPHA)',   // 青色
    'rgba(255, 152, 0, ALPHA)',   // 橙色
    'rgba(76, 175, 80, ALPHA)',   // 绿色
  ]
  ...
  function createParticle() {
    ...
    const edgeDist = Math.max(w, h) * 0.6 + Math.random() * 100
    const x = cx + Math.cos(angle) * edgeDist
    const y = cy + Math.sin(angle) * edgeDist
    const speed = 0.3 + Math.random() * 0.7
    const size = 1.5 + Math.random() * 3
    const colorTemplate = colors[Math.floor(Math.random() * colors.length)]
    const life = 0  // 0 → 1 表示从边缘到中心的进度

    return { x, y, speed, size, colorTemplate, life, angle, edgeDist }
  }
  ...
  for (let i = 0; i < PARTICLE_COUNT; i++) {
    ...
    p.life = Math.random()  // 随机初始进度，避免同时出现
    // 根据 life 计算当前位置
    const remaining = 1 - p.life
    p.x = cx + Math.cos(p.angle) * p.edgeDist * remaining
    p.y = cy + Math.sin(p.angle) * p.edgeDist * remaining
    _particles.push(p)
  }
    ...
    for (let i = 0; i < _particles.length; i++) {
      ...
      const alpha = p.life < 0.15
        ...
        : p.life > 0.85
          ? (1 - p.life) / 0.15
          : 1
      const finalAlpha = alpha * 0.8

      // 绘制光点（带光晕）
      const color = p.colorTemplate.replace('ALPHA', finalAlpha.toFixed(2))
      const glowColor = p.colorTemplate.replace('ALPHA', (finalAlpha * 0.3).toFixed(2))
...
function stopConvergeAnimation() {
  if (_animFrameId) {
