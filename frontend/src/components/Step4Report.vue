      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <div class="action-buttons">
            <button v-if="isComplete" class="action-btn primary" @click="handleDownloadReport" :disabled="isDownloading">
              <span>{{ isDownloading ? '下载中...' : '下载文档' }}</span>
...
const handleDownloadReport = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      emit('add-log', `报告下载失败: ${res.error}`)
    }
  } catch (err) {
    emit('add-log', `报告下载异常: ${err.message}`)
  } finally {
