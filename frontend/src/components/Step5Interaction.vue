        ...
        <div v-if="reportOutline" class="report-content-wrapper">
          ...
          <div class="report-header-block">
            <div class="report-meta">
              <span class="report-tag">Retrospection Report</span>
              <span class="report-id">ID: {{ reportId || 'REF-2024-X92' }}</span>
            </div>
            <h1 class="main-title">{{ reportOutline.title }}</h1>
            <p class="sub-title">{{ reportOutline.summary }}</p>
            <div class="report-actions">
              <button class="download-btn" @click="handleDownloadReport" :disabled="isDownloading" title="下载报告">
                ...
                <span>{{ isDownloading ? '下载�?..' : '下载文档' }}</span>
              </button>
            </div>
        ...
        <div class="action-bar-header">
          ...
          <div class="action-bar-tabs">
            ...
            <button 
              ...
              :disabled="!reportOutline || isDownloading"
              :title="!reportOutline ? '报告尚未加载' : '下载报告'"
            >
              ...
              <span v-if="isDownloading" class="download-spinner-small"></span>
              <span>{{ isDownloading ? '下载�? : '下载文档' }}</span>
            </button>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">报告生成智能体的快速对话版本，可调�?4 种专业工具，拥有MiroFish的完整记�?/div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">对齐现实世界种子数据与模拟环境状态，结合Global/Local Memory机制，提供跨时空的深度归因分�?/div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">基于图结构的广度遍历算法，重构事件传播路径，捕获全量信息流动的拓扑结�?/div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">QuickSearch 快速检�?/div>
                ...
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">自主式访谈，能够并行与模拟世界中个体进行多轮对话，采集非结构化的观点数据与心理状�?/div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="chatTarget === 'agent' && selectedAgent" class="agent-profile-card">
            ...
            <div v-if="showFullProfile && selectedAgent.bio" class="profile-card-body">
              <div class="profile-card-bio">
                <div class="profile-card-label">简�?/div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? '�?Report Agent 对话，深入了解报告内�? : '与模拟个体对话，了解他们的观�? }}
              </p>
            </div>
            <div 
        ...
        <div v-if="activeTab === 'survey'" class="survey-container">
          ...
          <div class="survey-setup">
            <div class="setup-section">
              <div class="section-header">
                ...
                <span class="selection-count">已�?{{ selectedAgents.size }} / {{ profiles.length }}</span>
              </div>
              ...
              <div class="selection-actions">
                <button class="action-link" @click="selectAllAgents">全�?/button>
          ...
          <div v-if="surveyResults.length > 0" class="survey-results">
            <div class="results-header">
              ...
              <span class="results-count">{{ surveyResults.length }} 条回�?/span>
            </div>
...
const chatHistoryCache = ref({}) // 缓存所有对话记�? { 'report_agent': [], 'agent_0': [], 'agent_1': [], ... }
...
// 保存当前对话记录到缓�?const saveChatHistory = () => {
...
const selectReportAgentChat = () => {
  ...
  // 恢复 Report Agent 的对话记�?  chatHistory.value = chatHistoryCache.value['report_agent'] || []
}
...
const selectAgent = (agent, idx) => {
  ...
  // 恢复�?Agent 的对话记�?  chatHistory.value = chatHistoryCache.value[`agent_${idx}`] || []
...
const renderMarkdown = (content) => {
  ...
  // 处理列表 - 支持子列�?  html = html.replace(/^(\s*)- (.+)$/gm, (match, indent, text) => {
  ...
  // 清理列表项之间的所有空�?  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // 清理列表开始标签后的空�?  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  ...
  // 清理 <p><br> 紧跟块级元素的情况（多余空行导致�?  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // 清理连续�?<br> 标签
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // 清理块级元素后紧跟的段落开始标签前�?<br>
...
const sendMessage = async () => {
  ...
  try {
    ...
    scrollToBottom()
    // 自动保存对话记录到缓�?    saveChatHistory()
  }
}

const sendToReportAgent = async (message) => {
  addLog(`�?Report Agent 发�? ${message.substring(0, 50)}...`)
  ...
  if (res.success && res.data) {
    chatHistory.value.push({
      role: 'assistant',
      content: res.data.response || res.data.answer || '无响�?,
      timestamp: new Date().toISOString()
    })
    addLog('Report Agent 已回�?)
  } else {
...
const sendToAgent = async (message) => {
  if (!selectedAgent.value || selectedAgentIndex.value === null) {
    throw new Error('请先选择一个模拟个�?)
  }
  
  addLog(`�?${selectedAgent.value.username} 发�? ${message.substring(0, 50)}...`)
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? '提问�? : '�?}�?{msg.content}`)
      .join('\n')
    prompt = `以下是我们之前的对话：\n${historyContext}\n\n现在我的新问题是�?{message}`
  }
  ...
  if (res.success && res.data) {
    // 正确的数据路�? res.data.result.results 是一个对象字�?    // 格式: {"twitter_0": {...}, "reddit_0": {...}} 或单平台 {"reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData
    
    // 将对象字典转换为数组，优先获�?reddit 平台的回�?    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // 优先使用 reddit 平台回复，其�?twitter
...
const submitSurvey = async () => {
  ...
  addLog(`发送问卷给 ${selectedAgents.value.size} 个对�?..`)
  ...
  try {
    ...
    if (res.success && res.data) {
      // 正确的数据路�? res.data.result.results 是一个对象字�?      // 格式: {"twitter_0": {...}, "reddit_0": {...}, "twitter_1": {...}, ...}
      const resultData = res.data.result || res.data
      const resultsDict = resultData.results || resultData
      ...
      for (const interview of interviews) {
        ...
        if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
          ...
          if (agentResult) {
            responseContent = agentResult.response || agentResult.answer || '无响�?
          }
        } else if (Array.isArray(resultsDict)) {
          // 兼容数组格式
          const matchedResult = resultsDict.find(r => r.agent_id === agentIdx)
          if (matchedResult) {
            responseContent = matchedResult.response || matchedResult.answer || '无响�?
          }
        }
...
const downloadReportAsMarkdown = () => {
  ...
  let md = `# ${reportOutline.value.title}\n\n`
  md += `> ${reportOutline.value.summary}\n\n---\n\n`
  ...
  a.href = url
  a.download = `${reportOutline.value.title || 'report'}.md`
    ...
    if (res.success && res.data) {
      ...
      logs.forEach(log => {
        ...
        if (log.action === 'section_complete' && log.section_index < 100 && log.details?.content) {
          // content 可能是对�?{title, content, ...} 或纯字符�?          const raw = log.details.content
          const text = typeof raw === 'object' ? (raw.content || JSON.stringify(raw)) : raw
          generatedSections.value[log.section_index] = text
        }
      })
...
const handleDownloadReport = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`报告下载失败: ${res.error}`)
    }
  } catch (err) {
    addLog(`报告下载异常: ${err.message}`)
  } finally {
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`加载�?${profiles.value.length} 个模拟个体`)
    }
  } catch (err) {
...
/* Left Panel - Report Style (�?Step4Report.vue 完全一�? */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
