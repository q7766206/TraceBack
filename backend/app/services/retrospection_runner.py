...
class RetrospectionRunner:
    ...
    def __init__(self, analysis_id: str, graph_id: str):
        ...
        try:
            ...
            logger.warning(f"LLM API Key未配置，部分功能将不可用: {e}")
    ...
    def _run_analysis(self):
        ...
        try:
            if not self.llm:
                logger.error(f"[{self.analysis_id}] LLM API Key未配置，无法执行分析")
                self.manager.update_status(self.analysis_id, AnalysisStatus.FAILED, error="LLM API Key未配置，无法执行分析")
                return
    ...
    def _call_agent(self, agent_id: str, user_message: str, context: str = "") -> str:
        """调用Agent获取文本响应"""
        if not self.llm:
            raise ValueError("LLM API Key未配置，无法调用Agent")
