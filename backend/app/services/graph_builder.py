...
class GraphBuilderService:
        ...
        try:
            ...
            self._zep_wait_for_episodes(
                ...
                lambda msg, prog: self.task_manager.update_task(
                    task_id, progress=60 + int(prog * 30), message=msg
                ),
                graph_id=graph_id,
            ...
            logger.error(f"Zep 图谱构建失败: {e}", exc_info=True)
            import traceback as tb
            tb.print_exc()
            self.task_manager.update_task(task_id, status=TaskStatus.FAILED, message=str(e))
    ...
    def _zep_wait_for_episodes(
        ...
        while pending and (time.time() - start_time) < timeout:
            ...
            if progress_callback:
                ...
                progress_callback(f"Zep 处理中: {completed}/{total} 完成 {profile_msg}", prog)
    ...
    def _build_graph_local_worker(
        ...
        try:
            ...
            logger.error(f"本地图谱构建失败: {e}", exc_info=True)
            self.task_manager.update_task(task_id, status=TaskStatus.FAILED, message=str(e))
