...
logger = get_logger('traceback.report_agent')
...
class InsightForgeResult:
    ...
    def to_text(self) -> str:
        return "\n".join(self.insights) if self.insights else "未找到相关洞察"
...
class InterviewResult:
    ...
    def to_text(self) -> str:
        ...
        for r in self.responses:
            parts.append(f"[{r.get('agent', '未知')}]: {r.get('response', '')}")
        return "\n".join(parts)


class GraphToolsService:
    """
    图谱工具服务（替代旧版 ZepToolsService）
    基于 LocalGraphStore + SearchEngine 提供检索能力
    """
    ...
    def insight_forge(self, graph_id: str, query: str,
                      simulation_requirement: str = "", report_context: str = "") -> InsightForgeResult:
        """深度分析：从图谱中检索相关信息并生成洞察"""
        try:
            results = self.graph_store.keyword_search(graph_id, query, top_k=15)
            insights = []
            for node in results.get("nodes", [])[:10]:
                name = node.get("name", "") if isinstance(node, dict) else node.name
                summary = node.get("summary", "") if isinstance(node, dict) else node.summary
                if summary:
                    insights.append(f"[{name}] {summary}")
            for edge in results.get("edges", [])[:10]:
                fact = edge.get("fact", "") if isinstance(edge, dict) else edge.fact
                if fact:
                    insights.append(fact)
            return InsightForgeResult(query=query, insights=insights)
        except Exception as e:
            logger.warning(f"insight_forge 失败: {e}")
            return InsightForgeResult(query=query)

    def panorama_search(self, graph_id: str, query: str,
                        include_expired: bool = True) -> PanoramaResult:
        """全景搜索：图谱关键词搜索 + 网络搜索"""
        items = []
        try:
            graph_results = self.graph_store.keyword_search(graph_id, query, top_k=10)
            for node in graph_results.get("nodes", [])[:5]:
                name = node.get("name", "") if isinstance(node, dict) else node.name
                summary = node.get("summary", "") if isinstance(node, dict) else node.summary
                items.append({"title": name, "snippet": summary, "source": "知识图谱"})
        except Exception as e:
            logger.warning(f"panorama_search 图谱检索失败: {e}")

        try:
            web_results = self.search_engine.search(query, max_results=5)
            for r in web_results:
                items.append({"title": r.title, "snippet": r.snippet, "source": r.url})
        except Exception as e:
            logger.warning(f"panorama_search 网络搜索失败: {e}")

        return PanoramaResult(items=items)

    def quick_search(self, graph_id: str, query: str, limit: int = 10) -> PanoramaResult:
        """快速搜索：仅图谱关键词搜索"""
        items = []
        try:
            results = self.graph_store.keyword_search(graph_id, query, top_k=limit)
            for node in results.get("nodes", []):
                name = node.get("name", "") if isinstance(node, dict) else node.name
                summary = node.get("summary", "") if isinstance(node, dict) else node.summary
                items.append({"title": name, "snippet": summary, "source": "知识图谱"})
        except Exception as e:
            logger.warning(f"quick_search 失败: {e}")
        return PanoramaResult(items=items)

    def get_graph_statistics(self, graph_id: str) -> Dict[str, Any]:
        """获取图谱统计信息"""
        try:
            return self.graph_store.get_stats(graph_id)
        except Exception as e:
            logger.warning(f"get_graph_statistics 失败: {e}")
            return {"node_count": 0, "edge_count": 0}

    def get_entity_summary(self, graph_id: str, entity_name: str) -> Dict[str, Any]:
        """获取实体摘要"""
        try:
            results = self.graph_store.keyword_search(graph_id, entity_name, top_k=5)
            ...
            if nodes:
                ...
                return {
                    "name": node.get("name", "") if isinstance(node, dict) else node.name,
                    "summary": node.get("summary", "") if isinstance(node, dict) else node.summary,
                    "labels": node.get("labels", []) if isinstance(node, dict) else node.labels,
                }
            return {"name": entity_name, "summary": "未找到该实体", "labels": []}
        except Exception as e:
            logger.warning(f"get_entity_summary 失败: {e}")
            return {"name": entity_name, "summary": "查询失败", "labels": []}

    def interview_agents(self, simulation_id: str, interview_requirement: str,
                         simulation_requirement: str = "", max_agents: int = 5) -> InterviewResult:
        """采访Agent（当前版本返回空结果，待后续实现）"""
        logger.info(f"interview_agents: 功能待实现 (simulation_id={simulation_id})")
        return InterviewResult()

    def get_entities_by_type(self, graph_id: str, entity_type: str, limit: int = 20) -> List[Dict]:
        """按类型获取实体"""
        try:
            all_nodes = self.graph_store.get_all_nodes(graph_id)
            matched = []
            ...
            logger.warning(f"get_entities_by_type 失败: {e}")
            return []

    def get_simulation_context(self, graph_id: str, simulation_id: str,
                               simulation_requirement: str) -> str:
        """获取模拟上下文"""
        try:
            stats = self.graph_store.get_stats(graph_id)
            return (
                f"图谱统计: {stats.get('node_count', 0)} 个实体, {stats.get('edge_count', 0)} 条关系\n"
                f"模拟需求: {simulation_requirement}"
            )
        except Exception as e:
            return f"模拟需求: {simulation_requirement}"
...
class ReportAgent:
    ...
    def __init__(
        ...
        self.llm = llm_client or LLMClient(preset="default")
    ...
    def _generate_section_simple(
        ...
        """
        简化版章节生成：直接调用LLM生成内容，不使用ReACT循环
        ...
        Returns:
            章节内容（Markdown格式）
        """
        logger.info(f"简化生成章节: {section.title}")
...
请直接撰写章节正文内容，使用Markdown格式，不要添加任何标题。
"""
        ...
        try:
            ...
            if response is None:
                logger.error(f"章节 {section.title} 生成失败：LLM 返回空响应")
                return "（本章节生成失败：LLM 返回空响应，请稍后重试）"
            
            # 直接返回生成的内容
            return response.strip()
        except Exception as e:
            logger.error(f"章节 {section.title} 生成失败: {str(e)}")
            return f"（本章节生成失败：{str(e)}，请稍后重试）"
    ...
    def generate_report(
        ...
        try:
            ...
            for i, section in enumerate(outline.sections):
                ...
                # 简化章节生成：直接调用LLM生成内容，不使用ReACT循环
                section_content = self._generate_section_simple(
                    section=section,
                    outline=outline,
