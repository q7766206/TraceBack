...
def chat_with_report():
    ...
    try:
        ...
        if simulation_id:
            ...
            if run_result:
                actions = run_result.get("actions", [])[:30]
                context_lines = []
                for a in actions:
                    if a.get("content"):
                        context_lines.append(f"[@{a.get('agent_name','?')}] {a.get('content','')[:100]}")
                context = "\n".join(context_lines)

        llm = LLMClient()
        prompt = f"""你是 TraceBack 回溯分析系统的报告助手。用户正在查看一份历史事件因果分析报告。
