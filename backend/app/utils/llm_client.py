...
class LLMClient:
    ...
    # 已知不支持 response_format 的模型列表（阿里云百炼、火山引擎等）
    _KNOWN_NO_JSON_FORMAT_MODELS = {
        'qwen-max', 'qwen-max-2025-01-25', 'qwen-plus', 'qwen-turbo', 'qwen-long',
        'deepseek-v3-2-251201', 'doubao-seed-2-0-pro-260215', 'doubao-seed-2-0-mini-260215',
    ...
    def chat_json(
        ...
        """
        发送聊天请求并返回JSON
        对于不支持的模型，直接使用纯 prompt 模式
        """
        # 构造纯 prompt 的消息
        def _make_json_messages():
            json_messages = [m.copy() for m in messages]
            if json_messages:
                json_messages[-1]["content"] = json_messages[-1]["content"] + "\n\n请严格以JSON格式返回结果，不要包含任何其他文字说明。"
            return json_messages

        # 检查模型是否已知不支持 json_object
        if self.model in self._KNOWN_NO_JSON_FORMAT_MODELS or self.model in self._no_json_format_models:
            logger.debug(f"模型 {self.model} 已知不支持 response_format，直接使用纯 prompt 模式")
            response = self.chat(
                messages=_make_json_messages(),
                temperature=temperature,
                max_tokens=max_tokens,
            )
            return self._parse_json_response(response)

        # 第一次尝试：带 response_format（仅对已知支持的模型）
        try:
            response = self.chat(
                messages=messages,
                temperature=temperature,
            ...
            if "response_format" in err_str or "json_object" in err_str or "400" in err_str:
    ...
    def _parse_json_response(self, response: str) -> Dict[str, Any]:
        ...
        if first_brace != -1 and last_brace > first_brace:
            ...
            try:
                ...
                # 4.4 修复未转义的引号
                # 尝试修复 "key": "value" "next_key" 这种情况
                candidate_fixed = re.sub(r'("\s*\})\s*("\s*:)', r'\1,\2', candidate_fixed)
                
                try:
                    return json.loads(candidate_fixed)
                except json.JSONDecodeError:
                    # 4.5 尝试使用 json5 或更宽松的解析器
                    try:
                        import demjson3
                        return demjson3.decode(candidate_fixed)
                    except ImportError:
                        pass
                    except Exception:
                        pass
        ...
        # 6. 最后的尝试：使用修复工具
        try:
            fixed_json = self._fix_json_with_repair(cleaned)
            if fixed_json:
    ...
    def _fix_json_with_repair(self, json_str: str) -> Optional[str]:
        """使用多种策略修复损坏的JSON"""
        # 尝试使用 json_repair 库（如果安装）
        try:
            import json_repair
